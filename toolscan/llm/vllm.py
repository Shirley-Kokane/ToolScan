...
class VLLM:
    def __init__(self,
        ...
        if self.context_length > 8192:
            self.llm = LLM(model=self.model, dtype=d_type,  gpu_memory_utilization=0.9, max_num_batched_tokens=8192, max_model_len=8192) #tensor_parallel_size=ngpu,
        else:
            self.llm = LLM(model=self.model, dtype=d_type,  gpu_memory_utilization=0.9, max_num_batched_tokens=self.context_length, tensor_parallel_size=ngpu) #tensor_parallel_size=ngpu,
