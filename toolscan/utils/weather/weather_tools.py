...
def clean_observation(observation):
    ...
    if (
        type(new_observation) == dict
        and "daily" in new_observation.keys()
        and "temperature_2m_max" in new_observation["daily"].keys()
    ):
...
class weather_toolkits:
    ...
    def get_historical_temp(
        ...
        if is_historical is True:
            ...
            if start_date is not None:
                ...
                if start_date_obj >= current_date_obj:
                    return (
                        False,
                        "Error: start_date should be earlier than current_date",
                    )
        ...
        if is_within_30_days(start_date, end_date) is False:
            return (
                False,
                "Error: Sorry, at present, we support a maximum time span of 30 days between start_date and end_date in a single query. Your input exceeds this range. You can split your current query into multiple sub-queries that meet our criteria.",
            )
    ...
    def get_snow_forecast(
        self, latitude=None, longitude=None, start_date=None, end_date=None
    ):
        ...
        success, response = self.get_historical_snow(
            latitude=latitude,
            longitude=longitude,
            start_date=start_date,
            end_date=end_date,
    ...
    @log_path
    def get_current_snow(self, latitude=None, longitude=None, current_date=None):
        success, response = self.get_historical_snow(
            latitude=latitude,
            longitude=longitude,
    ...
    def get_current_temp(self, latitude=None, longitude=None, current_date=None):
        ...
        success, response = self.get_historical_temp(
            latitude=latitude,
            longitude=longitude,
            start_date=current_date,
            end_date=current_date,
    ...
    def get_rain_forecast(
        ...
        success, response = self.get_historical_rain(
            latitude=latitude,
            longitude=longitude,
            start_date=start_date,
            end_date=end_date,
    ...
    @log_path
    def get_current_rain(self, latitude=None, longitude=None, current_date=None):
        success, response = self.get_historical_rain(
            latitude=latitude,
            longitude=longitude,
            start_date=current_date,
    ...
    @log_path
    def get_distance(
        self, latitude1=None, longitude1=None, latitude2=None, longitude2=None
    ):
    ...
    @log_path
    def get_historical_air_quality_index(
        self,
        latitude=None,
        longitude=None,
        start_date=None,
        ...
        def _gather_data(response):
            ...
            for i in range(num_days):
                european_aqi_pm2_5.append(
                    new_response["daily"]["european_aqi_pm2_5"][24 * i + 12]
                )
    ...
    @log_path
    def get_current_air_quality_index(
        self, latitude=None, longitude=None, current_date=None
    ):
        success, response = self.get_historical_air_quality_index(
            latitude=latitude,
            longitude=longitude,
            start_date=current_date,
            end_date=current_date,
...
if __name__ == "__main__":
    init_config = {"current_date": "2023-01-01"}
    tool = weather_toolkits(init_config=init_config)

    print(
        tool.get_historical_temp(
            latitude=31.22222,
            longitude=121.45806,
