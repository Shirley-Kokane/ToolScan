...
headers = {
    "Authorization": "Bearer {}".format(
        os.environ["MOVIE_KEY"] if "MOVIE_KEY" in os.environ.keys() else ""
    ),
}


def clean_observation(observation: Union[List[Dict[str, Any]], Dict[str, Any]]):
...
class movie_toolkits:
    ...
    def get_movie_crew(self, movie_id=None):
        ...
        if response.status_code == 200:
            ...
            return_data = {
                "crew": [
                    {"id": crew["id"], "name": crew["name"], "job": crew["job"]}
                    for crew in data["crew"]
                    if crew["job"] in must_contain_job
                ]
            }
    ...
    def get_person_crew(self, person_id=None):
        ...
        if response.status_code == 200:
            ...
            return_data = {
                "crew": [
                    {"id": crew["id"], "title": crew["title"], "job": crew["job"]}
                    for crew in data["crew"][:10]
                ]
            }
            return True, return_data
        else:
    ...
    def get_person_external_ids(self, person_id=None):
        url = URLS["people"]["external_ids"].format(person_id=person_id)
        response = requests.get(url, headers=headers)
        if response.status_code == 200:
            data = response.json()
    ...
    def get_movie_alternative_titles(self, movie_id=None):
        url = URLS["movies"]["alternative_titles"].format(movie_id=movie_id)
        response = requests.get(url, headers=headers)
        if response.status_code == 200:
            data = response.json()
    ...
    def get_movie_translation(self, movie_id=None):
        url = URLS["movies"]["translation"].format(movie_id=movie_id)
        response = requests.get(url, headers=headers)
        if response.status_code == 200:
            data = response.json()
            ...
            return_data["translations"] = [
                item
                for item in data["translations"]
                if item["iso_3166_1"] in target_lang
            ]
